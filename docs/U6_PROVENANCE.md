...
| Inter-UAV safety mechanism | discrete-time CBF-style projection/shield | Literature-backed concept + research adaptation | Safe-RL/CBF UAV literature uses a safety filter between learned actions and realized dynamics and notes that the policy still needs a corrective/buffer signal. This implementation avoids a new QP dependency: it minimally projects unsafe one-step pair candidates, rechecks constraints, counts uniquely corrected UAVs, logs correction distance, and applies `-safety_reward_coeff * correction_m / d_safe`; the exact scale remains a project calibration. |
