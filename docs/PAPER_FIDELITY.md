...
`configs/scenarios/u6.yaml` and `u9.yaml` deliberately separate the root-paper reproduction from homogeneous research scenarios. They use identical multi-rotors, full 3D motion, altitude-aware Bayesian sensing, a ground command station, persistent application-level DTN buffering, and UavNetSim MAC/PHY execution. The action is six-dimensional: horizontal thrust, horizontal-acceleration azimuth, vertical acceleration, transmit gate, transmit power, and immediate recipient/next hop.
...
Several combinations remain original research adaptations: mapping Liu's abstract sensing levels to the physical 50/100/150 m levels, the fixed 50 x 50 belief grid, the exact six-coordinate hybrid action, closed-slot UavNetSim admission, the launch-pad geometry, and the exact edge-GCS coordinate. Initial confirmation uses accumulated posterior plus persistent direct fine evidence; only post-expiry report regeneration requires a fresh fine positive. Peer belief sync uses a symmetric one-byte log-odds codec so `p=0.5` is exact and decoded values never become absorbing endpoints. Concurrent peer fan-in reserves application-buffer room in deterministic sender order. GCS link shaping is proportional to newly ACKed report bytes, peer hops cannot earn positive shaping, and both pairwise shield displacement and world-boundary/obstacle correction are penalized. A newly false-confirmed empty cell receives the symmetric negative task event. Depleted UAVs receive zero reward and terminate individually, while horizon truncation continues to bootstrap. The former constant peer heading slot now reports the previous recipient-bin center without changing observation dimensions; scientific runs must therefore retrain despite shape-compatible checkpoints.

Khan, Yanmaz, and Rinner's [ICRA 2014 occupancy-map merging paper](https://pervasive.uni-klu.ac.at/BR/pubs/2014/Khan_ICRA2014.pdf) was consulted for probabilistic information-merging and detection-error context. [P-DQN](https://arxiv.org/abs/1810.06394) and the UAV-specific [MAHyAR paper](https://doi.org/10.1109/LWC.2026.3663841) were consulted only as evidence for a possible future explicit hybrid-action actor; no MASAC, MADDPG, or MATD3 implementation is changed here. Obstacle count/radii and all exact reward/energy coefficients remain assumptions or calibration-required choices rather than paper values.
