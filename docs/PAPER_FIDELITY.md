...
`configs/scenarios/u6.yaml` and `u9.yaml` deliberately separate the root-paper reproduction from homogeneous research scenarios. They use identical multi-rotors, full 3D motion, altitude-aware Bayesian sensing, a ground command station, persistent application-level DTN buffering, and UavNetSim MAC/PHY execution. The action is six-dimensional: Cartesian acceleration `(a_x, a_y, a_z)`, binary transmit gate, continuous transmit power, and immediate recipient/next hop.

The change does **not** retroactively make these elements part of the Ao et al. root paper. Each research-scenario subsystem parameter has its own provenance. Strong examples are Liu et al.'s repeated Bayesian sensing and 0.9/0.8/0.7 detection profiles, Hu et al.'s continuous planar sensing geometry, Du et al.'s 1 MB report and 300 s TTL references, JUROR's DTN lifecycle/team-reward features, Wang et al.'s communication-attempt penalty concept, UavNetSim's native lower-layer defaults, and published baselines for 50/100/150 m flight levels, 2 km WLAN radius, 0.1--0.4 W transmit power and 77 Wh battery capacity. The 3 MB buffer, exact reward coefficients, closed 1 s packet-admission contract and normalized safety-correction penalty are explicit research adaptations.

Several combinations remain original research adaptations: mapping Liu's abstract sensing levels to the physical 50/100/150 m levels, the fixed 50 x 50 belief grid, the exact six-coordinate hybrid action `[a_x,a_y,a_z,tx_gate,tx_power,recipient]`, closed-slot UavNetSim admission, the launch-pad geometry, and the exact edge-GCS coordinate. The motion block is direct Cartesian acceleration so zero is neutral; gate/recipient are hard-execution coordinates canonicalized before replay/critic use with a straight-through gradient relaxation, and MASAC entropy covers only the four continuous controls. Initial confirmation uses accumulated posterior plus persistent direct fine evidence; only post-expiry report regeneration requires a fresh fine positive. Peer belief sync uses a symmetric one-byte log-odds codec so `p=0.5` is exact and decoded values never become absorbing endpoints. Concurrent peer fan-in reserves application-buffer room in deterministic sender order. GCS link shaping is proportional to newly ACKed report bytes, peer hops cannot earn positive shaping, and both pairwise shield displacement and world-boundary/obstacle correction are penalized. A newly false-confirmed empty cell receives the symmetric negative task event. A UAV already inactive at step start contributes no trainable transition; a UAV that depletes during the step receives its final transition reward and then becomes terminal, with later joint-replay rows validity-masked out of actor/critic losses. Because remaining mission time is observable and JUROR models the task as finite-horizon with episode termination at `T_max`, the 600-step `u6`/`u9` mission deadline is terminal rather than an external truncation; legacy root-paper scenarios keep truncation semantics. The former constant peer heading slot now reports the previous recipient-bin center without changing observation dimensions. The subsequent Cartesian-motion and hybrid-action canonicalization also changes action meaning without changing tensor width; scientific U6/U9 runs must therefore retrain from scratch rather than resume older shape-compatible checkpoints.

Khan, Yanmaz, and Rinner's [ICRA 2014 occupancy-map merging paper](https://pervasive.uni-klu.ac.at/BR/pubs/2014/Khan_ICRA2014.pdf) was consulted for probabilistic information-merging and detection-error context. [P-DQN](https://arxiv.org/abs/1810.06394) and the UAV-specific [MAHyAR paper](https://doi.org/10.1109/LWC.2026.3663841) support treating the U6/U9 interface as hybrid rather than fully continuous. The current MASAC/MADDPG/MATD3 baselines retain their original network families but canonicalize the binary gate/recipient semantics before critic/replay use with a straight-through relaxation; this is not claimed to be an exact categorical/parameterized-action implementation. Obstacle count/radii and all exact reward/energy coefficients remain assumptions or calibration-required choices rather than paper values.
