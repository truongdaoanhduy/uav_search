...
Several combinations remain original research adaptations: mapping Liu's abstract sensing levels to the physical 50/100/150 m levels, the fixed 50 x 50 belief grid, the exact six-coordinate hybrid action, closed-slot UavNetSim admission, the launch-pad geometry, and the exact edge-GCS coordinate. Initial confirmation uses accumulated posterior plus persistent direct fine evidence; only post-expiry report regeneration requires a fresh fine positive. GCS link shaping is proportional to newly ACKed report bytes, peer hops cannot earn positive shaping, and safety-shield displacement is penalized. Obstacle count/radii and all exact reward/energy coefficients remain assumptions or calibration-required choices rather than paper values.
