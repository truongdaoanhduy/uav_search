...
Target confirmation and mission delivery are separate: altitude-dependent sensing updates each UAV's **local** posterior map. A cell is declared when any active UAV's own posterior reaches `>= 0.99`; detector selection is deterministic (highest posterior, then shortest horizontal distance, then lowest UAV index). Belief maps are **not globally fused**. A peer can obtain another UAV's state/belief/target knowledge only after the MARL policy selects that peer and the network backend successfully delivers a 4 KiB synchronization bundle. The receiver then performs minimum-uncertainty cell-wise fusion into its own map only. Failed transmissions, topology snapshots, and disconnected UAVs do not change peer knowledge; same-step multi-hop information propagation is prevented with frozen slot-start snapshots. The 4 KiB synchronization size is a project research assumption rather than a paper-explicit serialization format. The current application baseline uses 1 MB reports, 100 MB UAV buffers and 300 s TTL from UAV-assisted VDTN literature; UavNetSim packetizes offered bytes at 1024 B while the application may offer multiple packets per 1 s RL macro-step. Relay forwarding remains limited to one application-level hop per macro-step. `info`, CSV and W&B include altitude, belief entropy, target posterior, scanned cells, positive sensor observations, information gain, true/false confirmations, topology, queues, PDR, throughput, delay and radio energy. The `u6`/`u9` battery baseline is 77 Wh = 277.2 kJ (Mavic 3 Enterprise literature reference). There is **no charging**: a depleted UAV is disabled for motion, sensing, TX, RX and relaying for the rest of the episode. See [`docs/U6_PROVENANCE.md`](docs/U6_PROVENANCE.md) for the exact paper-backed vs adapted split.
